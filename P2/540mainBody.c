/*
    540mainBody.c
    Implements setting body uniforms using 540body.c
...
On macOS, make sure that NUMDEVICEEXT below is 2, and then compile with 
    clang 540mainBody.c -lglfw -lvulkan
...
int bodyNum = 3;
bodyBody landscapeBody, waterBody, heroBody;
...
int initializeScene() {
    ...
    bodyConfigure(&landscapeBody, &landVesh);
...
void setBodyUniforms(uint32_t imageIndex) {
    ...
    isoSetRotation(&heroBody.isometry, rot);
    isoSetTranslation(&heroBody.isometry, heroPos);

    bodySetUniforms(&landscapeBody, &aligned, 0);
...
int initializeCommandBuffers() {
    ...
    for (size_t i = 0; i < swap.numImages; i += 1) {
        ...
        bodyRender(&landscapeBody, &connCommandBuffers[i], &connPipelineLayout, &(desc.descriptorSets[i]), &aligned, 0);
        bodyRender(&waterBody, &connCommandBuffers[i], &connPipelineLayout, &(desc.descriptorSets[i]), &aligned, 1);
        bodyRender(&heroBody, &connCommandBuffers[i], &connPipelineLayout, &(desc.descriptorSets[i]), &aligned, 2);
