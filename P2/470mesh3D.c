/*
    ...
    Modified from 250mesh3D.c to use floats instead of doubles (and uint16_ts instead of ints in some places).
...
void mesh3DSmoothNormals(meshMesh *mesh, int n) {
    int i;
    uint16_t *tri;
    float *a, *b, *c, normal[3] = {0.0, 0.0, 0.0};
    /* Zero the normals. */
...
the input. */
void mesh3DRotateVector(float theta, const float v[2], float vRot[2]) {
    float cosTheta = cos(theta);
    float sinTheta = sin(theta);
    float vRot0 = cosTheta * v[0] - sinTheta * v[1];
...
int mesh3DInitializeRevolution(
        meshMesh *mesh, int zNum, const float z[], const float r[], 
        const float t[], int sideNum) {
    int i, j, error;
...
int mesh3DInitializeSphere(
        meshMesh *mesh, float r, int layerNum, int sideNum) {
    int error, i;
    float *ts = (float *)malloc((layerNum + 1) * 3 * sizeof(float));
    if (ts == NULL)
        return 1;
    else {
        float *zs = &ts[layerNum + 1];
        float *rs = &ts[2 * layerNum + 2];
        for (i = 0; i <= layerNum; i += 1) {
            ts[i] = (float)i / layerNum;
...
int mesh3DInitializeCapsule(
        meshMesh *mesh, float r, float l, int layerNum, int sideNum) {
    int error, i;
    float theta;
    float *ts = (float *)malloc((2 * layerNum + 2) * 3 * sizeof(float));
    if (ts == NULL)
        return 1;
    else {
        float *zs = &ts[2 * layerNum + 2];
        float *rs = &ts[4 * layerNum + 4];
        zs[0] = -l / 2.0;
        rs[0] = 0.0;
        ts[0] = 0.0;
        for (i = 1; i <= layerNum; i += 1) {
            theta = M_PI / 2.0 * (3 + i / (float)layerNum);
            zs[i] = -l / 2.0 + r + r * sin(theta);
            rs[i] = r * cos(theta);
            ts[i] = (zs[i] + l / 2.0) / l;
        }
        for (i = 0; i < layerNum; i += 1) {
            theta = M_PI / 2.0 * i / (float)layerNum;
...
int mesh3DInitializeCylinder(meshMesh *mesh, float r, float l, int sideNum) {
    ...
    for (int i = 0; i < sideNum; i += 1) {
        attr[0] = r * cos(2.0 * M_PI * (float)i / sideNum);
        attr[1] = r * sin(2.0 * M_PI * (float)i / sideNum);
...
float zs[3][3] = {
    ...
    {6.0, 5.0, 3.0}, 
    {4.0, 3.0, -1.0}};
int error = mesh3DInitializeLandscape(&mesh, 3, 20.0, (float *)zs); */
int mesh3DInitializeLandscape(
        meshMesh *mesh, int size, float spacing, const float *data) {
    int i, j, error;
    int a, b, c, d;
    float *vert, diffSWNE, diffSENW;
        ...
        for (i = 0; i < size; i += 1)
            for (j = 0; j < size; j += 1) {
                ...
                vec8Set(i * spacing, j * spacing, data[i * size + j], 
                    (float)i, (float)j, 0.0, 0.0, 0.0, vert);
            }
        /* Build the triangles. */
...
int mesh3DInitializeDissectedLandscape(
        meshMesh *mesh, const meshMesh *land, float angle, int noMoreThan) {
