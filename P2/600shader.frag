...
void main() {
    ...
    vec4 cDiffPositional = iDiffusePositional * scene.cLightPositional * rgbaTex;
    vec4 cSpec = pow(iSpec, shininess) * scene.cLight * body.cSpecular;
    vec4 cSpecPositional = pow(iSpecPositional, shininess) * scene.cLightPositional * body.cSpecular;
