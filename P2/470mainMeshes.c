/*
    470mainMeshes.c
    A demo showing a render of two veshes, one capsule and one box.
...
On macOS, make sure that NUMDEVICEEXT below is 2, and then compile with 
    clang 470mainMeshes.c -lglfw -lvulkan
You might also need to compile the shaders with 
    glslc 470shader.vert -o 470vert.spv
    glslc 460shader.frag -o 460frag.spv
Then run the program with 
    ./a.out

On Linux, make sure that NUMDEVICEEXT below is 1, and then compile with 
    clang 470mainMeshes.c -lglfw -lvulkan -lm
You might also need to compile the shaders with 
    /mnt/c/VulkanSDK/1.3.216.0/Bin/glslc.exe 470shader.vert -o 470vert.spv
...
/* The variable to hold the shader program. */
shaProgram shaProg;
...
/* Now, only a veshStyle variable (initialized in initializeArtwork) is needed to pass this mesh style to Vulkan. */
veshStyle styleVesh;

/* Our veshVeshes which will store information about the meshes we'd like to render in a format readable for Vulkan. The contents of these veshVeshes is written in initializeArtwork. */
veshVesh veshBox, veshCapsule;


/* Initializes the artwork. Upon success (return code 0), don't forget to 
finalizeArtwork later. */
int initializeArtwork() {
    // our temporary 3D meshes
    meshMesh meshBox, meshCapsule;
    if (shaInitialize(&shaProg, "470vert.spv", "460frag.spv") != 0) {
        return 7;
    }
    if (veshInitializeStyle(&styleVesh, MESHNUMATTRS, meshAttrDims) != 0) {
        shaFinalize(&shaProg);
        return 6;
    }
    if (mesh3DInitializeBox(&meshBox, -2.0, 0.0, -1.0, 1.0, -2.0, -1.0) != 0) {
        veshFinalizeStyle(&styleVesh);
        shaFinalize(&shaProg);
        return 5;
    }
    if (veshInitializeMesh(&veshBox, &meshBox) != 0) {
        meshFinalize(&meshBox);
        veshFinalizeStyle(&styleVesh);
        shaFinalize(&shaProg);
        return 4;
    }
    if (mesh3DInitializeCapsule(&meshCapsule, 1.0, 3.0, 50.0, 50.0) != 0) {
        veshFinalize(&veshBox);
        meshFinalize(&meshBox);
        veshFinalizeStyle(&styleVesh);
        shaFinalize(&shaProg);
        return 3;
    }
    if (veshInitializeMesh(&veshCapsule, &meshCapsule) != 0) {
        meshFinalize(&meshCapsule);
        veshFinalize(&veshBox);
        meshFinalize(&meshBox);
        veshFinalizeStyle(&styleVesh);
        shaFinalize(&shaProg);
...
/* Build the connection between the swap chain and the scene (artwork, etc.). At this point there are two pieces of machinery: the pipeline and the command buffers. */
...
int initializeCommandBuffers() {
    ...
    for (size_t i = 0; i < swap.numImages; i += 1) {
        ...
        /* Render two meshes which are actually veshes. */
        veshRender(&veshBox, connCommandBuffers[i]);
        veshRender(&veshCapsule, connCommandBuffers[i]);
...
int initializeConnection() {
    if (initializePipeline(&shaProg, &styleVesh.vertexInputInfo, &styleVesh.inputAssembly) != 0)
        return 2;
...
int reinitializeSwapChain() {
    ...
    /* Release the connection before releasing the swap chain. */
    finalizeConnection();
    swapFinalize(&swap);
    if (swapInitialize(&swap) != 0)
        return 2;
    /* Create the connection after creating the swap chain. */
...
int presentFrame() {
    ...
    /* We now have one command buffer instead of zero command buffers. */
...
int main() {
    ...
    /* Initialize the connection after the swap chain and scene. */
    if (initializeConnection() != 0) {
        finalizeArtwork();
        swapFinalize(&swap);
        vulFinalize(&vul);
    ...
    guiRun(&gui);
    vkDeviceWaitIdle(vul.device);
    /* Finalize the connection before the scene and swap chain. */
    finalizeConnection();
    finalizeArtwork();
    swapFinalize(&swap);
    vulFinalize(&vul);
